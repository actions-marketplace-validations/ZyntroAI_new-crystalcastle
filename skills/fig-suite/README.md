...
**One source of truth.** The gate iterates `policy["criteria"]` — it never
hard-codes the list. Add a criterion to the policy without registering a check
and the gate reports `no check implemented for <ID>`, an explicit failure
rather than a silent pass.

**The `.fig/` contract.** A project declares its state in small files:
`context.yaml`, `routing.yaml`, `evidence.json`, `external-writes.yaml`,
`verify.json`, `handoff.yaml`. The gate reads them; absent files are meaningful
(`external-writes.yaml` missing means no external writes, which passes).

**Dependencies.** Pure standard library. PyYAML is used when present; a JSON
fallback covers the policy file otherwise, so the loader never hard-fails on a
bare interpreter.

**Testing.** `python -m pytest tests/` runs the suite. The tests assert the
clean example passes, the broken example fails on exactly the criteria it is
built to fail, and the policy and manifest agree.
